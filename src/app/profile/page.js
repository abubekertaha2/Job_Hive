...
//       // We need to handle the case where the profile data is null, which is expected for new users.
//       // The optional chaining operator (`?.`) prevents crashes and handles this correctly.
//       setProfileData(prev => ({
//         ...prev,
...
//       // Set the image preview to the existing URL or a blank string
...
//       // Set the resume preview to the existing URL or a blank string
...
//       }
//     } else {
//       // If file input is cleared, reset preview to original data or empty
//       if (name === 'profileImage') {
...
//       // If a new URL is returned from the server, update the state and preview.
...
//       // We don't need to re-fetch if there was a profile_picture_url
//       // The re-fetch logic is fine, but it might not be strictly necessary
//       // if the URL is returned in the response. I've left it as is since it's a valid fallback.

//       // Reset the file inputs to clear them from the form
...
//       {/* Adding a key here forces the form to re-render, preventing state issues on dynamic data */}
//       <form key={userData.id} onSubmit={handleSubmit} className="space-y-6">
...
//               {/* Adding a key here forces the Image component to re-render when the URL changes */}
//               <Image
//                 key={profileImagePreview || 'default-image'}
