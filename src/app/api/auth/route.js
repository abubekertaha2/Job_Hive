...
export async function POST(req) {
  ...
  try {
    ...
    if (action === 'signup') {
      const [existingUsers] = await connection.execute('SELECT id FROM user WHERE email = ?', [email]);
      if (existingUsers.length > 0) {
      ...
      const newUserId = result.insertId;
      const token = jwt.sign({ id: newUserId, email, role }, JWT_SECRET, { expiresIn: '7d' });

      const response = NextResponse.json({
        message: 'User registered successfully!',
        user: { id: newUserId, email, name, role, company }
      }, { status: 201 });
